...
module.exports = async (params) => {
    ...
    // Step 1: 选择目标目录（即分类：博文 / 公告）
    const allFolders = app.vault.getAllLoadedFiles()
        .filter(f => f.children && !f.path.includes('/') && !f.path.startsWith('.') && f.path !== 'saike_model')
        .map(f => f.path);

    const folderOptions = [...allFolders];
    const category = await quickAddApi.suggester(folderOptions, folderOptions);
    if (!category) {
        new Notice("已取消：未选择分类", NOTICE_TIMEOUT);
        return;
    }
    ...
    const folderPath = `${category}/${articleName.trim()}`;
    const filePath = `${folderPath}/index.md`;

    // Step 3: 输入作者名
    const authorName = await quickAddApi.inputPrompt("输入作者名", "Saike Lab");
    if (!authorName || !authorName.trim()) {
        new Notice("已取消：未输入作者名", NOTICE_TIMEOUT);
        return;
    }
    ...
    // Step 6: 读取模板并替换占位符
    const templateFile = app.vault.getAbstractFileByPath("saike_model/saike_model.md");
    if (!templateFile) {
        new Notice("模板文件不存在：saike_model/saike_model.md", NOTICE_TIMEOUT);
        return;
    }
    ...
    // Slug: saike-年份-序号
    const slugPattern = /^saike-(\d{4})-(\d{3})$/;
    let maxNum = 0;
    for (const file of app.vault.getMarkdownFiles()) {
        const cache = app.metadataCache.getFileCache(file);
    ...
    const nextSlug = `saike-${year}-${String(maxNum + 1).padStart(3, '0')}`;

    content = content.replace(/\{\{date:.*?\}\}/g, dateStr);
    content = content.replace(/\{\{title\}\}/g, articleName.trim());
    content = content.replace(/\{\{author\}\}/g, authorName.trim());
